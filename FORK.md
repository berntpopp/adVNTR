...
| `capabilities --json`, `fit-background`, `replay-frameshift` | **Supported installed calibration interfaces; see [CALIBRATION.md](CALIBRATION.md)** |
...
|---|---|
| 2.4.1 | Background-fit diagnostic reports accept JSON null for unknown VNTR length and retain it as a separate stratum. Missing length is never imputed. Recipe-v1 fitted probabilities, fold assignments, genotype decisions and defaults are unchanged. Complete v2 fitting and installed wheel/sdist tests cover unknown lengths. |
| 2.4.0 | Installed calibration and replay. Adds verified build/source capabilities, run-local frameshift policies, complete v2 evidence capture with anonymous occurrence identities and verified model/background snapshots, and `replay-frameshift` using the production decision functions. The installed background fitter no longer depends on a source checkout; recipe-v1 estimation remains fixed while CV/replay uses an explicit diagnostic policy. Complete capture checks preserve zero-event denominators and audit occurrence attribution. Existing caller defaults stay unchanged; no trained background or calibrated default operating point is introduced. See [CALIBRATION.md](CALIBRATION.md) for contracts and limitations. |
| 2.3.0 | Adapter read-through filtering (Issue #4), callable cluster bound validation & repeat-unit label decoupling (Issue #7), and model update refusal (Issue #9). Adds `--filter-adapter-readthrough` and `--min-read-match-ratio` (Tier B) to eliminate false-positive frameshift calls driven by Illumina adapter sequence in short inserts, with compound event rejection, split-alignment motif detection, and opportunity denominator integrity. Adds `CallableCluster` checking cluster size limits ($E \le 49$ diploid / $98$ haploid) to prevent likelihood ratio inversion. Introduces `RepeatUnitLabelMap` to decouple internal 0-indexed HMM states from external repeat unit labels with atomic remapping and ambiguous label rejection. Fail-fast refusal of `-u/--update` on CLI with informative error. `vntr_finder.py` reduced to 1184 LOC; coverage ratchet increased to 44.0%; full test suite expanded to 594 tests. |
| 2.2.0 | Exact frameshift caller (Task 8), background model fitter (Task 8c), and rare-unit coverage guard (Task 9). Adds `advntr fit-background` command to fit, screen, cross-validate, and emit frozen background models from calibration sinks. Implements exact one-sided binomial tail test for frameshift genotyping behind opt-in `--exact-frameshift-caller` with `--frameshift-background` (Tier B). Implements `--rare-unit-coverage-guard` to prevent false-positive calls when sequencing coverage collapses on rare repeat units. Full in-tree modularization with all modules under 650 LOC; test suite expanded to 556 tests with 42.0% coverage. |
