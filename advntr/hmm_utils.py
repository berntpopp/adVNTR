...
from advntr import hmm_construction
...
def get_prefix_matcher_hmm(pattern, maximum_error_rate=None):
    """Compatibility default; explicit callers isolate all model probabilities."""
    rate = settings.MAX_ERROR_RATE if maximum_error_rate is None else maximum_error_rate
    return hmm_construction.get_prefix_matcher_hmm(pattern, rate)


def get_suffix_matcher_hmm(pattern, maximum_error_rate=None):
    """Compatibility default; explicit callers isolate all model probabilities."""
    rate = settings.MAX_ERROR_RATE if maximum_error_rate is None else maximum_error_rate
    return hmm_construction.get_suffix_matcher_hmm(pattern, rate)


def get_constant_number_of_repeats_matcher_hmm(patterns, copies, vpaths, maximum_error_rate=None):
    """Keep read-alignment reconstruction separate from model construction."""
    rate = settings.MAX_ERROR_RATE if maximum_error_rate is None else maximum_error_rate
    return hmm_construction.get_constant_number_of_repeats_matcher_hmm(
        patterns, copies, vpaths, rate, get_multiple_alignment_of_repeats_from_reads)
...
def get_repeat_matcher_enhanced_hmm(pattern_clusters, copies, vpaths, maximum_error_rate=None):
    """Keep the legacy call surface while forwarding one rate to construction."""
    rate = settings.MAX_ERROR_RATE if maximum_error_rate is None else maximum_error_rate
    return hmm_construction.get_repeat_matcher_enhanced_hmm(
        pattern_clusters, copies, vpaths, rate, get_multiple_alignment_of_repeats_from_reads)
...
def get_read_matcher_model_enhanced(left_flanking_region, right_flanking_region, patterns, copies=1, vpaths=None,
                                    is_frameshift_mode=False, maximum_error_rate=None):
    """Resolve the compatibility rate once; nested builders receive it explicitly."""
    rate = settings.MAX_ERROR_RATE if maximum_error_rate is None else maximum_error_rate
    return hmm_construction.get_read_matcher_model_enhanced(
        left_flanking_region, right_flanking_region, patterns, copies, vpaths,
        is_frameshift_mode, rate, get_multiple_alignment_of_repeats_from_reads)
